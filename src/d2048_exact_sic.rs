...
pub fn welch_report() -> String {
    ...
    format!(
        "  Welch saturation, measured on the recovered d=12 fiducial\n           overlaps checked      {} (= d^2 - 1)\n           target 1/(d+1)        {:.17}\n           min |<psi|D psi>|^2   {:.17}\n           max |<psi|D psi>|^2   {:.17}\n           worst deviation       {:.3e}  (f64; the same vector gives 4.5e-201 at 200 digits)\n           source                ~/imsgct/d12_sic_build, d12_psi_uhi.pkl\n\n           At d=2048 the moduli come from the exact extraction rather than from a\n           search: `d2048 exact` prints the unit, its two reciprocal embeddings and\n           the monomial. A displacement check needs the phase assignment across the\n           orbit, which is a different object from the moduli, so this verb reports\n           what it measures at 12 and leaves 2048 to the verb that computes it.",
