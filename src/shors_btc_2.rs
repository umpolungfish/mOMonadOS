...
use crate::moDOT_alchemy::{
    MoDoTAlchemyPipeline, EcPoint, ec_mul
};
use crate::fibonacci_shor::{ShorCircuitParams, certify_advantage};

// Import the secp256k1 types we need from pk2sk
use crate::pk2sk::{U256, N_LIMBS, GX_LIMBS, GY_LIMBS};
...
fn grammar_winding_extract(public_key: &EcPoint) -> (Option<U256>, ShorCircuitParams, Vec<i32>) {
    ...
    // ⊢: Fixed-point oneshot check (k = PK.x, PK.y, n-PK.x, n-PK.y if < 2^32)
    // Use the constants from moDOT_alchemy
    let G = EcPoint::new(
        crate::moDOT_alchemy::SECP256K1_GX.clone(),
        crate::moDOT_alchemy::SECP256K1_GY.clone()
    );
    let n = crate::moDOT_alchemy::SECP256K1_N.clone();
    ...
    for cand in &candidates {
        if cand.0[1] == 0 && cand.0[2] == 0 && cand.0[3] == 0 {
            ...
            if kG.equals(public_key) {
                return (Some(k), ShorCircuitParams::new(12, 2, 15), vec![]);
            }
        }
    }
    
    // ⊢: Small-multiple check (k <= 999)
    for k_val in 1..=999 {
        let k = U256::from_u64(k_val);
        let kG = ec_mul(&k, &G);
        if kG.equals(public_key) {
            return (Some(k), ShorCircuitParams::new(12, 2, 15), vec![]);
        }
...
pub fn run_shors_btc_2(public_key: &EcPoint) -> ShorsBtc2Result {
    ...
    let _order = crate::moDOT_alchemy::SECP256K1_N.clone();
    ...
    ShorsBtc2Result {
        ...
        measurement_count: 2,
        qft_circuit: Some(qft_circuit(12, false)),
...
impl ShorsBtc2Result {
    ...
    pub fn print_report(&self) {
        ...
        if let Some(ref params) = self.shor_params {
            sprintln!("Shor Params: n_qubits={}, work_qubits={}, total_qubits={}, est_braid_len={}",
                params.n_qubits, params.n_work_qubits, params.n_total_qubits, params.estimated_braid_len
            );
        }
        ...
        if let Some(ref braid) = self.qft_braid {
            sprintln!("IQFT Braid (Fibonacci): {} generators, strands={}", 
                braid.len(),
                crate::fibonacci_shor::strands_for_qubits(12)
            );
        }
