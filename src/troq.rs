...
pub fn expand_axis(slot: &str) -> Vec<&'static str> {
    match slot {
        ...
        "Ð" => vec!["𐑛 (wedge/0D)", "𐑨 (ash/2D)", "𐑼 (array/∞D)", "𐑦 (if'/holographic)"],
        "Þ" => vec!["𐑡 (network)", "𐑰 (inclusion)", "𐑥 (bowtie)", "𐑶 (box)", "𐑸 (holographic)"],
...
pub fn help_text() -> &'static str {
    "TROQ — Triple-Ramified Ouroboric Quantale\n\
     ...
     troq axis <slot>  expand a primitive axis (⊙, Φ, Ω, Ð, ...)\n\
